...
class PersonnelSystem:
    def __init__(self, root):
        ...
        self.columns_tab2 = [
            ...
            "英文姓名", "職稱", "人員類別", "性別", "薪點", 
            "最高學位", "最高學歷國別", "最高學歷", "出生年月日", "到校日期", 
            "戶籍地址", "名冊地址", "現居地址", "通訊電話", "校外電子信箱", 
            "國籍", "總量管制-學校(一)", "總量管制-系所(一)", "總量管制-學位(一)", 
            "官職等", "官等", "EMI", "學術專長及研究【以35字為限(含標點符號)】"
        ]
    ...
    def setup_tab7(self):
        ...
        self.health_columns_map = {
            ...
            "職稱": ["職稱", "職別"],
            "出生年月日": ["出生年月日", "出生日期"],
            "西元出生年月日": ["出生年月日", "出生日期"], # 共用原始欄位，後續邏輯轉換
            "到校日期": ["到校日期"],
    ...
    def export_health_center_excel(self):
        ...
        try:
            ...
            # 西元轉換邏輯
            def convert_to_western(val):
                if pd.isna(val) or str(val).strip() == '': return ""
                s = str(val).strip().split('.')[0]
                # 假設格式為 7 位數民國年 (YYYMMDD) 或 6 位數 (YYMMDD)
                try:
                    if len(s) >= 5:
                        year_len = len(s) - 4
                        minguo_year = int(s[:year_len])
                        western_year = minguo_year + 1911
                        return f"{western_year}/{s[year_len:year_len+2]}/{s[year_len+2:]}"
                    return s
                except:
                    return s
            ...
            for target in selected_targets:
                ...
                if actual_col:
                    if target == "西元出生年月日":
                        final_data[target] = res[actual_col].apply(convert_to_western)
                    else:
