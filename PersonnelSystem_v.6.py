...
class PersonnelSystem:
    ...
    def setup_tab7(self):
        ...
        input_f = ttk.LabelFrame(main, text="條件設定", padding=15)
        input_f.pack(fill=tk.X, pady=10)
        
        # 到校日期輸入
        row1 = ttk.Frame(input_f)
        row1.pack(fill=tk.X, pady=5)
        ttk.Label(row1, text="1. 到校日期 (YYYYMMDD):", font=('Microsoft JhengHei', 11)).pack(side=tk.LEFT, padx=5)
        ...
        ttk.Label(row2, text="2. 匯出檔案名稱:", font=('Microsoft JhengHei', 11)).pack(side=tk.LEFT, padx=5)
    ...
    def export_health_center_excel(self):
        ...
        try:
            ...
            # 過濾邏輯
            # 確保日期是字串且符合比較 (處理可能讀入為數值的狀況)
            def clean_date(val):
                if pd.isna(val): return ""
                v = str(val).strip().split('.')[0]
                return v

            df['temp_arrival'] = df[arrival_col].apply(clean_date)
            ...
            # 提取指定欄位：姓名、服務單位、一級單位、出生年月日、到校日期、本校電子信箱
            target_cols_map = {
                "姓名": ["姓名", "憪"],
                "服務單位": ["服務單位", "單位"],
                "一級單位": ["一級單位"],
            ...
            final_data = {}
            for target, keywords in target_cols_map.items():
                col = find_col(keywords)
                if col:
                    final_data[target] = res[col]
                else:
                    final_data[target] = "" # 找不到則留空
